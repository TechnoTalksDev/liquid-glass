  ...
  <div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div class="bg-white/10 backdrop-blur-sm rounded-lg p-6 text-white">
        <h2 class="text-2xl font-semibold mb-4">Background Content</h2>
        <p class="mb-4">
          This is the background content that will be captured and displayed behind 
          the liquid glass effect. The glass creates a beautiful distortion and 
          refraction of this content.
        </p>
        <div class="w-full h-32 bg-gradient-to-r from-yellow-400 to-red-500 rounded-lg mb-4"></div>
        <p>
          The liquid glass component uses Three.js and WebGL to create realistic 
          glass materials with proper light transmission, refraction, and dispersion.
        </p>
      </div>

      <div class="bg-white/10 backdrop-blur-sm rounded-lg p-6 text-white">
        <h2 class="text-2xl font-semibold mb-4">More Background</h2>
        <div class="space-y-4">
          <div class="w-full h-4 bg-blue-400 rounded"></div>
          <div class="w-3/4 h-4 bg-green-400 rounded"></div>
          <div class="w-1/2 h-4 bg-purple-400 rounded"></div>
          <div class="w-full h-4 bg-pink-400 rounded"></div>
        </div>
        <p class="mt-4">
          Background elements are captured in real-time and used as textures 
          for the glass background, creating an authentic glass effect.
        </p>
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-6 justify-center items-center">
      ...
      >
        <div class="text-center">
          <h3 class="text-xl font-bold text-white mb-4">Liquid Glass</h3>
    ...
    <!-- Controls -->
    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-6 text-white">
      <h3 class="text-lg font-semibold mb-4">Glass Style Controls</h3>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div>
          <strong>Depth:</strong> {currentGlassStyle.depth.toFixed(1)}
        </div>
        <div>
          <strong>Segments:</strong> {currentGlassStyle.segments}
        </div>
        <div>
          <strong>Radius:</strong> {currentGlassStyle.radius.toFixed(1)}
        </div>
        <div>
          <strong>Roughness:</strong> {currentGlassStyle.roughness.toFixed(2)}
        </div>
        <div>
          <strong>Transmission:</strong> {currentGlassStyle.transmission.toFixed(2)}
        </div>
        <div>
          <strong>Reflectivity:</strong> {currentGlassStyle.reflectivity.toFixed(2)}
        </div>
        <div>
          <strong>IOR:</strong> {currentGlassStyle.ior.toFixed(1)}
        </div>
        <div>
          <strong>Dispersion:</strong> {currentGlassStyle.dispersion.toFixed(1)}
        </div>
      </div>
    </div>
  </div>
